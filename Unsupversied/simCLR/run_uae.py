##run_uae.py-------perform re-training simCLR with UAE
##author: Chia-Yi Hsu
...
def perform_evaluation(estimator, input_fn, eval_steps, model, num_classes,hook,
  ...
  result = estimator.evaluate(
      input_fn, eval_steps,hooks=hook, checkpoint_path=checkpoint_path,
...
def main(argv):
  ...
  num_train_examples = 50000*2#builder.info.splits[FLAGS.train_split].num_examples
  num_eval_examples = 10000#builder.info.splits[FLAGS.eval_split].num_examples
  num_classes = 10#builder.info.features['label'].num_classes


  v = np.load(FLAGS.uae_path)
  train_data = np.append(data.train_data,v,axis=0)
  train_labels = np.argmax(data.train_labels,axis=1).astype('int64')
  ...
  if FLAGS.mode == 'eval':
    for ckpt in tf.train.checkpoints_iterator(
      ...
      try:
        print("num_classes:_________________", num_classes)
        ifn1, iterator_initializer_hook1 = data_lib.build_input_fn(data.test_data, np.argmax(data.test_labels,axis=1).astype('int64'), False)
        result = perform_evaluation(
            ...
            hook=[iterator_initializer_hook1],
    ...
    ifn, iterator_initializer_hook = data_lib.build_input_fn(train_data,train_labels, True)
    #estimator.train(
    #    data_lib.build_input_fn(train_data,train_labels, True), max_steps=train_steps)
    estimator.train(ifn,hooks=[iterator_initializer_hook], max_steps=train_steps)
